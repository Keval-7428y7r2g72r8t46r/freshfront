  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return error(`Sheets clear failed: ${res.status}`, res.status, text);
      }

      const data: any = await res.json().catch(() => ({}));
      return json({ clearedRange: String(data?.clearedRange || '') }, 200);
    } catch (e: any) {
      return error(e?.message || 'Sheets clear failed', 500);
    }
  },
};
