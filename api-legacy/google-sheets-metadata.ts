...
type SheetTab = { sheetId: number; title: string; index?: number };
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      const apiUrl = new URL(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(spreadsheetId)}`);
      apiUrl.searchParams.set('fields', 'spreadsheetId,properties.title,sheets(properties(sheetId,title,index))');
      ...
      if (!res.ok) {
        ...
        return error(`Sheets metadata failed: ${res.status}`, res.status, text);
      }

      const data: any = await res.json().catch(() => ({}));
      const sheets = Array.isArray(data?.sheets) ? data.sheets : [];
      const tabs: SheetTab[] = sheets
        ...
        .filter((t: SheetTab) => Number.isFinite(t.sheetId) && t.title);
      ...
      return error(e?.message || 'Sheets metadata failed', 500);
    }
  },
};
