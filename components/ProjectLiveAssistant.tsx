...
import { useAgentTaskSystem, AgentTask } from '../hooks/useAgentTaskSystem';
import { createPortal } from 'react-dom';
import ReactMarkdown from 'react-markdown';
import { GoogleGenAI, LiveServerMessage, Modality, StartSensitivity, EndSensitivity, createPartFromUri, Type, ThinkingLevel } from '@google/genai';
import { ResearchProject, KnowledgeBaseFile, SessionConversation, ChatMessage as PersistedChatMessage, BookAsset, BookPage, TableAsset, UploadedFile, SavedResearch, ProjectTask, ProjectNote, DualTheme, CapturedLead, LeadFormAsset, LeadFormField } from '../types';
...
import { computeSourceCount } from '../services/statsService';
...
import { findBusinessLeads, LeadSearchFilters } from '../services/leadServices';
...
export const ProjectLiveAssistant: React.FC<ProjectLiveAssistantProps> = ({
  ...
  // === TASK SYSTEM (useAgentTaskSystem Hook) ===
  // Simple task queue with persistence for tracking tool execution
  const taskSystem = useAgentTaskSystem(project?.id || 'default');

  // Compatibility shims for existing code
  const startPendingTask = useCallback((type: string, description: string) => {
    const task = taskSystem.addTask(type, description);
    taskSystem.startTask(task.id);
  }, [taskSystem]);

  const completePendingTask = useCallback(() => {
    taskSystem.completeTask();
  }, [taskSystem]);
...
Recent History: ${JSON.stringify(recentHistory.slice(-2))}`
  ...
  const executeSchedulePost = useCallback(async (
    ...
    // This matches UnifiedSocialPublisher logic EXACTLY
    ...
    if (postType !== 'TEXT' && mediaUrlArg) {
      // Check if URL needs to be uploaded to blob for public access
      const inputUrl = String(mediaUrlArg);
      if (inputUrl.includes('generativelanguage.googleapis.com') ||
        inputUrl.startsWith('data:') ||
        !inputUrl.startsWith('http')) {
        // Need to upload to Vercel Blob
        try {
          console.log('[Schedule] Uploading media to Vercel Blob...', inputUrl.substring(0, 50));
          const { upload } = await import('@vercel/blob/client');

          // Fetch the media
          const response = await fetch(inputUrl);
          const blob = await response.blob();
          const fileName = `scheduled-${Date.now()}.${contentTypeArg === 'video' ? 'mp4' : 'png'}`;
          const file = new File([blob], fileName, { type: blob.type });

          const blobResult = await upload(fileName, file, {
            access: 'public',
            handleUploadUrl: '/api/blob/upload',
          });

          mediaUrl = blobResult.url;
          console.log('[Schedule] Media uploaded to blob:', mediaUrl);
        } catch (uploadError: any) {
          console.error('[Schedule] Blob upload failed, using original URL:', uploadError);
          mediaUrl = inputUrl;
        }
      } else {
        mediaUrl = inputUrl;
      ...
      if (mediaUrl && contentType !== 'text') {
        ...
        if (mediaUrl.includes('generativelanguage.googleapis.com')) {
          ...
          if (att?.file) {
            ...
            try {
              const { upload } = await import('@vercel/blob/client');
              const result = await upload(att.file.name, att.file, {
                access: 'public',
                handleUploadUrl: '/api/blob/upload',
              });
              publicMediaUrl = result.url;
              console.log('[postToSocialPlatform] Uploaded to Vercel Blob:', result.url);
            } catch (blobError) {
  ...
  const generateProjectPodcastAsset = useCallback(
    async (prompt: string, style?: string, duration?: string) => {
      ...
      const durationValue: 'short' | 'medium' | 'long' =
        duration === 'short' || duration === 'long' ? duration : 'medium';
...
${highlights}`;
  ...
  const connectVoice = async () => {
    ...
    try {
      ...
      const baseVoiceInstruction = contextService.getProjectSystemInstruction(project, 'voice', userProfile);
...
${socialConnectionStatus}${voiceGeneratedMediaContext}`;
      ...
      const getTableDraftTool = {
        ...
        description: 'OPTIONAL: Get the current table being edited in the Assets > Tables tab (title/columns/rows). Only use this if you need to INSPECT the table structure. For EDITS, directly call set_table_cell, add_table_row, etc. based on user\'s request - the user can see the table and will reference columns by name.',
        parameters: {
          type: Type.OBJECT,
          properties: {},
        }
      ...
      const setTableCellTool = {
        name: 'set_table_cell',
        description: 'Set a cell value in the Tables tab editor by row/column index. Call this directly based on user\'s request - no need to call get_table_draft first. For bulk edits (e.g., "change entire column F to private"), call this tool MULTIPLE TIMES in a single response for each cell that needs updating.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            rowIndex: { type: Type.NUMBER, description: '0-based row index.' },
            colIndex: { type: Type.NUMBER, description: '0-based column index (column A=0, B=1, C=2, D=3, E=4, F=5, etc.).' },
      ...
      const fillTableColumnTool = {
        name: 'fill_table_column',
        description: 'Set ALL values in a specific column to the SAME value. Use this for bulk updates like "change column F to private".',
        parameters: {
          type: Type.OBJECT,
          properties: {
            colIndex: { type: Type.NUMBER, description: '0-based column index (A=0, B=1, C=2, D=3, E=4, F=5, etc.)' },
            value: { type: Type.STRING, description: 'The value to fill the entire column with.' }
          },
          required: ['colIndex', 'value']
        }
      };
      ...
      // --- TOOL: Find Business Leads ---
      const findBusinessLeadsTool = {
        name: 'find_business_leads',
        description: 'Find business leads (people) or companies using Wiza and Hunter. Use this when user asks to "find leads", "find people at X", "search for companies", etc. Can filter by job title, location, industry, or specific company/domain.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            jobTitle: { type: Type.STRING, description: 'Job title to search for (e.g. "Marketing Manager").' },
            location: { type: Type.STRING, description: 'Location (City, Country).' },
            industry: { type: Type.STRING, description: 'Industry (e.g. "Software", "Real Estate").' },
            companyName: { type: Type.STRING, description: 'Specific company name to search within.' },
            domain: { type: Type.STRING, description: 'Company domain (e.g. "google.com").' },
            keywords: { type: Type.STRING, description: 'General keywords.' }
          }
        }
      };
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: systemInstruction + '\n\nVOICE MODE INSTRUCTIONS:\n- Keep responses concise and conversational.\n- Do not read out long lists or IDs.\n- If you find leads, summarize the count and key examples rather than listing everyone.',
                  ...
                  if (fc.name === 'get_docs_draft') {
                    ...
                    try {
                      ...
                      try {
                        ...
                        const blob = await res.blob();
                        const file = new File([blob], `live-image-edit-${Date.now()}.png`, { type: blob.type || 'image/png' });
                        const kb = await storageService.uploadKnowledgeBaseFile(project.id, file);
                        kbFileId = kb.id;
                    ...
                    if (!topic) {
                      ...
                      try {
                        ...
                        const report = await performDeepResearch(topic);
                    ...
                    try {
                      ...
                      try {
                        ...
                        videoBlob = await generateVeoVideo(
                          ...
                          aspectRatio,
                          'speed',
                          [{ base64, mimeType: imageBlob.type || 'image/png' }]
                        );
                      }
...
`;
  ...
  const handleSendMessage = async (overrideInput?: string) => {
    const effectiveInput = typeof overrideInput === 'string' ? overrideInput : inputText;

    if ((!effectiveInput.trim() && readyAttachments.length === 0) || isProcessing || isGeneratingImage) return;
    ...
    // Re-Act Loop: Initialize from messages state
    let currentHistory: any[] = messages.map(m => ({
      role: m.role,
      text: m.text,
      parts: (m as any).parts || [{ text: m.text || ' ' }]
    }));
    // Build user message parts including file attachments for model analysis
    ...
    // Add fileData parts for each ready attachment (uploaded to Gemini Files API)
    // This enables the model to actually analyze PDFs, images, audio, and video
    for (const att of readyAttachments) {
      if (att.uploaded?.uri && att.uploaded?.mimeType) {
        userMessageParts.push({
          fileData: {
            fileUri: att.uploaded.uri,
            mimeType: att.uploaded.mimeType
          }
        });
      }
    }

    // Add to history if we have any parts (text or files)
    if (userMessageParts.length > 0) {
      currentHistory.push({ role: 'user', text: userMessage, parts: userMessageParts });
    }
...
If the user attaches a file (image, PDF, video, audio) to the message, you will receive it as a 'fileData' part.
- **PRIORITY:** You MUST read/analyze the attached file DIRECTLY using your multimodal capabilities.
- **FORBIDDEN:** Do NOT call 'get_project_file', 'analyze_project_file', or 'file_search' for files that are currently attached. These tools are ONLY for retrieving files from the project storage that are NOT attached to the current message.
- If user says "what is in this PDF?" -> Read the attached PDF directly.
- If user says "what is in the Guidelines PDF?" (and it's NOT attached) -> Use 'get_project_file'.
...
`
      ...
      const searchKnowledgeBaseChatTool = {
        name: 'search_knowledge_base',
        description: 'Search across ALL indexed documents in the project knowledge base to answer a question. Use this when the user asks a general question that may span multiple documents. DO NOT USE for files currently attached to the message - read those directly.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            query: {
      ...
      const getProjectFileTool = {
        name: 'get_project_file',
        description: 'Retrieve and analyze a specific file from the project by name. Use this when the user mentions a specific file name and wants information about it (e.g., "tell me about the image X", "analyze the file Y", "what is in the document Z"). This works for images (PNG, JPG), PDFs, and other uploaded files. IMPORTANT: DO NOT use this tool if the file is currently attached to the message (I can see it directly). Use this ONLY for files stored in the project that are NOT attached.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            fileName: {
...
Set useLastGenerated=true when user says "this" or "the image/video".`,
      ...
      // IMPORTANT: When user asks "generate a PDF", FIRST ask them:
      // "Would you like a higher quality PDF with images only (no selectable text), or an interactive PDF with selectable text?"
      // Then use the appropriate tool based on their answer.

      const generatePdfTool = {
        name: 'generate_pdf',
        description: `Generate a HIGH-QUALITY, illustrated PDF document with AI-generated images per page.

IMPORTANT: When user asks for "a PDF" or "generate PDF" without specifying type, FIRST ASK:
"Would you like:
1. **High-quality illustrated PDF** - Beautiful AI-generated images per page, but text is NOT selectable (great for visual impact)
2. **Interactive PDF** - Selectable text with rich styling (great for reading and copying text)"

Use THIS tool for option 1 (high-quality images). Features:
- AI-generates a unique image for each page
- Professional layout with visual storytelling
- Best for visual documents, lookbooks, photo-heavy content
- 4-24 pages, saved to project assets

TIP: If user says "continue" or agrees to a proposed topic, USE THAT TOPIC as the 'prompt'.`,
        parameters: {
          type: Type.OBJECT,
          properties: {
            prompt: {
              type: Type.STRING,
              description: 'Topic/Focus. If user said "continue", use the topic you just proposed.'
            },
            pageCount: {
              type: Type.NUMBER,
      ...
      const generateInteractivePdfTool = {
        name: 'generate_interactive_pdf',
        description: `Generate an INTERACTIVE HTML-based PDF with selectable text and professional styling.

IMPORTANT: When user asks for "a PDF" without specifying type, FIRST ASK about their preference (see generate_pdf description).

Use THIS tool when user wants option 2 (interactive/selectable text). Features:
- Selectable, copyable text
- Rich HTML styling with gradients, icons, modern design
- Print-ready layouts for any paper size
- Magazine-quality aesthetics
- No page count limit - content determines length
- Saved to project Assets
...
TIP: If user says "continue" or agrees to a proposed topic, USE THAT TOPIC as the 'prompt'.`,
        parameters: {
          ...
          properties: {
            ...
            documentType: {
              type: Type.STRING,
              description: 'Type of document to generate.',
              enum: ['ebook', 'guide', 'report', 'brochure', 'presentation', 'whitepaper', 'manual', 'magazine', 'portfolio']
            }
          },
          required: []
        }
      ...
      const findBusinessLeadsTool = {
        name: 'find_business_leads',
        description: 'Find real business leads (people or companies) using Wiza and Hunter.io. Use this when user asks to "find leads", "prospects", "email addresses", or "companies". Capable of finding specific people (e.g. "Software Engineers in SF") or companies (e.g. "SaaS companies in London"). Returns a list of potential leads with contact info.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            filters: {
              ...
              properties: {
                job_title: { type: Type.STRING, description: 'e.g. "Marketing Manager", "CEO"' },
                location: { type: Type.STRING, description: 'e.g. "New York", "United Kingdom"' },
                industry: { type: Type.STRING, description: 'e.g. "Software", "Real Estate"' },
                company_size: { type: Type.STRING, description: 'e.g. "51-200", "1000+"' },
                company_name: { type: Type.STRING, description: 'Specific company to search for people at' },
                domain: { type: Type.STRING, description: 'Specific domain to search (e.g. "stripe.com")' },
                keywords: { type: Type.STRING, description: 'General keywords for company discovery' }
              }
            },
            size: {
              type: Type.NUMBER,
              description: 'Number of leads to find (max 30). Default 10.'
            }
          },
          required: ['filters']
        }
...
DO NOT use schedule_post for email - use THIS tool instead.`,
      ...
      // Capture this state BEFORE the loop to track if we started with a search intent
      const initialNeedsBuiltInTools = needsGoogleSearch || shouldEnableFileSearch;

      // 2. Select Model
      // Use Gemini 2.5 Flash for reliable function calling. Gemini 3 + tools has compatibility issues.
      // This ensures tool calls work reliably while we wait for better Gemini 3 + tools support.
      ...
      // Robustly identify tool sets from the 'tools' array
      // Note: 'tools' is mutable (unshifted above), so we find components by property presence
      const functionDeclarationsTool = tools.find((t: any) => t.functionDeclarations);
      const googleSearchTool = tools.find((t: any) => t.googleSearch);
      const fileSearchTool = tools.find((t: any) => t.fileSearch);

      // Construct the separate tool sets
      // CRITICAL: Gemini API does NOT support combining googleSearch with fileSearch.
      // We must CHOOSE ONE based on user intent.
      const searchTools: any[] = [];
      if (shouldEnableFileSearch && fileSearchTool) {
        // User is asking about uploaded files/docs - use fileSearch ONLY
        searchTools.push(fileSearchTool);
        console.log('[ProjectLiveAssistant] Using fileSearch (document context detected)');
      } else if (needsGoogleSearch && googleSearchTool) {
        // User needs web search - use googleSearch ONLY
        searchTools.push(googleSearchTool);
        console.log('[ProjectLiveAssistant] Using googleSearch (web search intent detected)');
      }

      const functionTools = functionDeclarationsTool ? [functionDeclarationsTool] : [];

      console.log(`[ProjectLiveAssistant] Using model: ${targetModel}, Built-in Tools Needed: ${initialNeedsBuiltInTools}`);

      let keepLooping = true;
      while (keepLooping && loopCount < MAX_LOOPS) {
        keepLooping = false; // Reset

        // DYNAMIC TOOL SELECTION FOR RE-ACT LOOP
        // If we need search/file tools, use them in the first turn (loopCount 0).
        // For subsequent turns (or if search isn't needed), use function tools.
        let runtimeTools;
        if (loopCount === 0 && initialNeedsBuiltInTools) {
          runtimeTools = searchTools;
          console.log('[ProjectLiveAssistant] Re-Act Loop 0: Using Built-in Search Tools');
        } else {
          runtimeTools = functionTools;
          console.log(`[ProjectLiveAssistant] Re-Act Loop ${loopCount}: Using Function Tools`);
        }

        // Prepare History for API
        const apiContents = currentHistory.map(m => {
          if (m.parts) return m; // Already structured (e.g. function response)
          return {
        ...
        // gemini-2.5-flash is reliable, so no fallback needed currently.


        for (const attemptingModel of modelsToTry) {
          try {
            stream = await ai.models.generateContentStream({
              model: attemptingModel,
              contents: apiContents as any,
              config: {
                systemInstruction: systemInstruction,
                temperature: 1.0,
                maxOutputTokens: 8192,
                tools: runtimeTools,
                toolConfig: (loopCount === 0 && initialNeedsBuiltInTools) ? undefined : toolConfig,
                // Gemini 2.5 uses thinkingBudget, not thinkingLevel
                // -1 = dynamic thinking (default), 0 = off
                thinkingConfig: attemptingModel.includes('gemini-2.5')
                  ? { thinkingBudget: -1 } // Dynamic thinking
                  : attemptingModel.includes('gemini-3')
                    ? { thinkingLevel: ThinkingLevel.LOW }
                    : undefined,
              },
            });
            console.log(`[ProjectLiveAssistant] Successfully started stream with ${attemptingModel}`);
            break;
          } catch (e: any) {
            console.warn(`[ProjectLiveAssistant] Model ${attemptingModel} failed:`, e);
            if (attemptingModel === modelsToTry[modelsToTry.length - 1]) {
              console.error('[ProjectLiveAssistant] All models failed. Throwing last error.');
              throw e;
            }
            console.log('[ProjectLiveAssistant] Falling back to next model...');
          }
        }


        console.log(`[ProjectLiveAssistant] Iteration ${loopCount + 1}: Model request sent.`);

        let fullText = '';
        const aggregatedFunctionCalls: any[] = [];
        let latestGroundingMetadata: any = null;
        let modelResponsePartForHistory: any = { role: 'model', parts: [] };
        ...
        for await (const chunk of stream as any) {
          ...
          for (const part of parts) {
            if (part.text) {
              let textChunk: string = part.text;

              // Format Thoughts (Gemini 3)
              if ((part as any).thought) {
                // Ensure thoughts are visually distinct (e.g. italics or blockquote)
                // Since chunks are small, we just append. The model usually formats thoughts itself or we can wrap the whole block later.
                // For now, we rely on the model's text.
                // Optionally log it:
                // console.log('Thought chunk:', textChunk);
              }
        ...
        // CAPTURE FULL RESPONSE (Critical for Gemini 3 Thought Signatures)
        let finalParts: any[] = [];
        try {
          if (stream?.response) {
            const finalResponse = await stream.response;
            if (finalResponse?.candidates?.[0]?.content?.parts) {
              finalParts = finalResponse.candidates[0].content.parts;
              // Push to local history logic for Re-Act loop continuity
              currentHistory.push({ role: 'model', parts: finalParts });
              console.log('[ProjectLiveAssistant] Captured final parts with signatures:', finalParts.length);
            }
          }
        } catch (err) {
          console.warn('[ProjectLiveAssistant] Failed to retrieve final response object:', err);
          // Fallback to text if parts capture fails
          if (fullText) currentHistory.push({ role: 'model', parts: [{ text: fullText }] });
        }
        ...
        // Define here to ensure scope for the whole loop iteration
        const toolOutputsForHistory: any[] = [];

        if (aggregatedFunctionCalls.length > 0) {
          // Task Monitor: Start tracking pending task when processing tools
          const toolNames = aggregatedFunctionCalls.map((fc: any) => fc.name).join(', ');
          startPendingTask('tool_execution', `Processing: ${toolNames}`);
          ...
          for (const fc of aggregatedFunctionCalls) {
            ...
            // ------------------------------------------------------------------
            // RELIABILITY FIX: PDF Generation Routing
            // Ensure correct tool choice based on interactive vs standard preference
            // ------------------------------------------------------------------
            const pdfPrompt = String(args.prompt || '').toLowerCase();

            if (fc.name === 'generate_pdf') {
              // Check if user actually wanted interactive PDF
              const interactiveKeywords = ['interactive', 'selectable', 'copy text', 'copyable', 'html', 'rich text', 'links', 'clickable'];
              const wantsInteractive = interactiveKeywords.some(kw => pdfPrompt.includes(kw)) || pdfPrompt.includes('text selection');

              if (wantsInteractive) {
                console.log('[Routing Fix] 🔀 Redirecting generate_pdf -> generate_interactive_pdf based on intent');
                fc.name = 'generate_interactive_pdf';
              }
            }
            else if (fc.name === 'generate_interactive_pdf') {
              // Check if user actually wanted standard image-based PDF
              const standardKeywords = ['image only', 'images only', 'flat', 'not selectable', 'scanned', 'picture only', 'not interactive', 'standard pdf'];
              const wantsStandard = standardKeywords.some(kw => pdfPrompt.includes(kw));

              if (wantsStandard) {
                console.log('[Routing Fix] 🔀 Redirecting generate_interactive_pdf -> generate_pdf based on intent');
                fc.name = 'generate_pdf';
              }
            }
            ...
            if (fc.name === 'search_knowledge_base') {
            ...
            } else if (fc.name === 'find_business_leads') {
              const filters = args.filters || {};
              const size = args.size || 10;

              try {
                addMessage('model', 'Searching for business leads...'); // Optimistic update
                const result = await findBusinessLeads(filters, size);
                const provider = result.provider === 'hunter' ? 'Hunter.io' : 'Wiza';
                const items = result.data.profiles || result.data.companies || [];
                const total = result.data.total || items.length;

                let text = `## Leads Found (${items.length} of ${total})\n`;
                text += `**Source:** ${provider}\n\n`;

                if (items.length === 0) {
                  text += `No leads found matching criteria: ${JSON.stringify(filters)}`;
                } else {
                  // Table-like markdown representation
                  if (result.provider === 'wiza' || (result.meta && result.meta.type === 'domain_search')) {
                    // People results
                    items.forEach((p: any, idx: number) => {
                      const name = p.value || (p.first_name ? `${p.first_name} ${p.last_name || ''}` : 'Unknown');
                      const title = p.position || p.title || 'N/A';
                      const company = p.company || p.organization || filters.company_name || filters.domain || 'N/A';
                      const email = p.value || p.email || 'N/A';
                      text += `${idx + 1}. **${name}** - ${title} at ${company}\n`;
                      text += `   Email: ${email}\n`;
                      if (p.linkedin) text += `   LinkedIn: [View](https://linkedin.com/in/${p.linkedin})\n`;
                    });
                  } else {
                    // Company results (Hunter Discover)
                    items.forEach((c: any, idx: number) => {
                      const name = c.organization || c.name || c.domain;
                      const domain = c.domain || c.website;
                      const emails = c.emails_count ? `${c.emails_count.personal + c.emails_count.generic} emails` : '';
                      text += `${idx + 1}. **${name}** (${domain})\n`;
                      if (emails) text += `   Found: ${emails}\n`;
                      if (c.industry) text += `   Industry: ${c.industry}\n`;
                    });
                  }

                  text += `\n**Next Steps:**\n`;
                  text += `- You can ask me to "save these leads to a table" or "email these people".`;
                }
                ...
                const errText = `Failed to find leads: ${e?.message || e}`;
                if (!fullText.trim()) {
                  setMessages(prev => prev.map(m => m.id === streamingMessageId ? { ...m, text: (m.text || '') + (m.text ? '\n\n' : '') + errText, isGenerating: false } : m));
                } else {
                  addMessage('model', errText);
              ...
              try {
                ...
                try {
                  ...
                  try {
                    ...
                    const veoBlob = await generateVeoVideo(prompt, veoAspect, veoQuality, [{ base64: imageBase64, mimeType: imageMime }]);
                ...
                const successText = `I generated a Sora video based on your request and saved it to your project assets.\n\nVideo URL: ${videoUrl}`;

                setMessages(prev => {
                  // Remove streaming message and add final result
              ...
              try {
                ...
                if (!t) {
                  ...
                  // EAGER EDIT EXECUTION (User Request Compliance):
                  // If the user clearly wants to edit a column, do it NOW without waiting for another turn.
                  const info = `Table draft loaded:\n- title: ${t.title || '(untitled)'}\n- columns: ${(t.columns || []).length}\n- rows: ${(t.rows || []).length}`;
                  addMessage('model', info);

                  toolOutputsForHistory.push({
                    functionResponse: {
                      name: 'get_table_draft',
                ...
                const err = `Table draft read failed: ${String(e?.message || e)}`;
              ...
              try {
                ...
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.setCell(Number(args.rowIndex), Number(args.colIndex), String(args.value ?? ''));
                const msg = 'Updated a table cell in the Tables tab.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_cell', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to update table cell: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_cell', response: { error: err } } });
              }
            } else if (fc.name === 'add_table_row') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.addRow(typeof args.index === 'number' ? args.index : undefined);
                const msg = 'Added a row to the table.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'add_table_row', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to add row: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'add_table_row', response: { error: err } } });
              }
            } else if (fc.name === 'delete_table_row') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.deleteRow(Number(args.rowIndex));
                const msg = 'Deleted a row from the table.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'delete_table_row', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to delete row: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'delete_table_row', response: { error: err } } });
              }
            } else if (fc.name === 'add_table_column') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.addColumn(typeof args.name === 'string' ? args.name : undefined, typeof args.index === 'number' ? args.index : undefined);
                const msg = 'Added a column to the table.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'add_table_column', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to add column: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'add_table_column', response: { error: err } } });
              }
            } else if (fc.name === 'delete_table_column') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.deleteColumn(Number(args.colIndex));
                const msg = 'Deleted a column from the table.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'delete_table_column', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to delete column: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'delete_table_column', response: { error: err } } });
              }
            } else if (fc.name === 'rename_table_column') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.renameColumn(Number(args.colIndex), String(args.name ?? ''));
                const msg = 'Renamed a column in the table.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'rename_table_column', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to rename column: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'rename_table_column', response: { error: err } } });
              }
            } else if (fc.name === 'set_table_title') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.setTableTitle(String(args.title ?? ''));
                const msg = 'Updated the table title.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_title', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to set table title: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_title', response: { error: err } } });
              }
            } else if (fc.name === 'set_table_description') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                bridge.setTableDescription(String(args.description ?? ''));
                const msg = 'Updated the table description.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_description', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to set table description: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_description', response: { error: err } } });
              }
            } else if (fc.name === 'set_table_draft') {
              try {
                const bridge = getTableEditorBridge();
              ...
              try {
                ...
                rows[rowIndex] = row;
                bridge.setRows(rows);
                const msg = 'Replaced a full row in the table.';
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_row', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to replace row: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'set_table_row', response: { error: err } } });
              }
            } else if (fc.name === 'set_table_column') {
              try {
                const bridge = getTableEditorBridge();
                ...
                const msg = 'Replaced a full column in the table.';
                addMessage('model', msg);
                toolOutputsForHistory.push({
                  functionResponse: {
                    name: 'set_table_column',
            ...
            } else if (fc.name === 'fill_table_column') {
              try {
                const bridge = getTableEditorBridge();
                if (!bridge) throw new Error('Table editor is not available. Open the Assets > Tables tab and load/generate a table first.');
                const colIndex = Number(args.colIndex);
                ...
                bridge.setRows(nextRows);
                const msg = `Filled column ${colIndex} with value "${value}".`;
                addMessage('model', msg);
                toolOutputsForHistory.push({ functionResponse: { name: 'fill_table_column', response: { content: msg } } });
              } catch (e: any) {
                const err = `Failed to fill column: ${String(e?.message || e)}`;
                addMessage('model', err);
                toolOutputsForHistory.push({ functionResponse: { name: 'fill_table_column', response: { error: err } } });
              }
            } else if (fc.name === 'run_project_seo_analysis') {
              const keyword = String(args.keyword || args.prompt || '').trim() || project.name || 'SEO';
              const location = String(args.location || 'US').trim() || 'US';
              ...
              try {
                ...
                if (contentType === 'text' && isTextReferenceToMedia && readyAttachments.length > 0) {
                  const imageOrVideoAtt = readyAttachments.find(a =>
                    a.uploaded?.mimeType?.startsWith('image/') || a.uploaded?.mimeType?.startsWith('video/')
                  );
                  if (imageOrVideoAtt?.uploaded?.uri) {
                    // Auto-upgrade to media post
                    console.log('[post_to_social] Auto-detected media reference, upgrading from text to media post');
                ...
                if (contentType === 'text' && isTextReferenceToMedia && lastGeneratedAsset) {
                  console.log('[post_to_social] Auto-detected media reference to last generated asset');
              ...
              try {
                ...
                addMessage('model', `🔍 Searching for business leads matching your criteria...`);

                // Call the service
                const response = await findBusinessLeads(filters);
                const data = response.data || {};
                // Extract list from Wiza/Hunter response structure (profiles or companies)
                const results = Array.isArray(data) ? data : (data.profiles || data.companies || []);

                if (results && results.length > 0) {
                  const count = results.length;
                  const topResults = results.slice(0, 3).map((r: any) =>
                    `- ${r.name || r.companyName || r.fullName || r.company_name || 'Unknown'} (${r.title || r.industry || 'N/A'})`
                  ).join('\n');

                  addMessage('model', `✅ Found ${count} leads. Here are a few:\n${topResults}\n\n(Full list saved to your leads)`);

                  // Ideally we'd persist these to project.capturedLeads, 
                  // but for now we just show them.
                } else {
                  addMessage('model', `No leads found matching those criteria.`);
                }
              } catch (e: any) {
                console.error('find_business_leads error:', e);
                addMessage('model', `❌ Failed to find leads: ${e?.message || 'Unknown error'}`);
              }
...
*You can also find this in your Assets → PDFs tab.*`;
              ...
              // ========== Interactive PDF Generation Handler ==========
            } else if (fc.name === 'generate_interactive_pdf') {
              try {
                const prompt = String(args.prompt || '').trim();
                const documentType = String(args.documentType || 'guide');
                ...
                if (!prompt) {
                  addMessage('model', 'Please describe what the interactive PDF should be about.');
                  continue;
                }

                addMessage('model', `📄 Generating an interactive ${documentType} PDF about: "${prompt}"...\n\nThis will have selectable text and professional styling.`);
                ...
                // Refine the prompt
                const refinedPrompt = await refinePromptWithGemini3(
                  `Create an interactive, text-rich ${documentType} about: ${prompt}. 
                  Focus on: Clear headings, readable paragraphs, bullet points, and professional formatting.
                  The content should be comprehensive and well-structured.`,
                  ctx.fullContext,
                  'text'
                );

                // STEP 1: Generate Content Plan (Images Only)
                // We ask for JSON here because it's small and structured.
                const planResponse = await ai.models.generateContent({
                  model: 'gemini-2.5-flash',
                  contents: [{
                    role: 'user',
                    parts: [{
                      text: `Plan the visual content for an interactive ${documentType} about: ${refinedPrompt}

Return a JSON object with a list of 3-6 images to generate:
{
  "images": [
    { "id": "img1", "prompt": "Detailed description for the first image...", "caption": "Caption for the image" },
    { "id": "img2", "prompt": "Detailed description for the second image...", "caption": "Caption for the second image" }
  ]
}

Ensure the image prompts are specific and high-quality.`
                    }]
                  }],
                  config: {
                    temperature: 0.7,
                ...
                console.log('[Interactive PDF] Plan Response:', planText);
                let planData: { images: { id: string; prompt: string; caption: string }[] };
                ...
                const imageCount = planData.images?.length || 0;
                console.log(`[Interactive PDF] Plan parsed. Image count: ${imageCount}`);

                addMessage('model', `🎨 (2/3) Generating ${imageCount} AI images for your document...`);

                // STEP 2: Generate Images
                let previousImageBase64: string | null = null;
                const generatedImages: { id: string; url: string; caption: string }[] = [];
                const latestSession = project.researchSessions?.[project.researchSessions.length - 1];
                const sessionId = latestSession?.id || 'temp-session';

                if (planData.images && Array.isArray(planData.images)) {
                  for (let i = 0; i < planData.images.length; i++) {
                    const imgInfo = planData.images[i];
                    try {
                      console.log(`[Interactive PDF] Generating image ${i + 1}/${imageCount}: ${imgInfo.prompt}`);
                      const references: ImageReference[] = [];
                      if (previousImageBase64) {
                        references.push({ base64: previousImageBase64, mimeType: 'image/png' });
                      }

                      // Generate Image using current theme/style
                      const result = await generateImageWithReferences(imgInfo.prompt, references, { useProModel: true });
                      const imageUrl = result.imageDataUrl;
                      ...
                      const res = await fetch(imageUrl);
                      const blob = await res.blob();
                      const fileName = `interactive-pdf-${Date.now()}-${imgInfo.id}.png`;
                      const file = new File([blob], fileName, { type: blob.type || 'image/png' });
                      ...
                      console.error(`Failed to generate image ${imgInfo.id}:`, imgErr);
                    }
                  }
                }

                addMessage('model', `📝 (3/3) Writing content and embedding images...`);

                // STEP 3: Generate HTML (Text Mode)
                const htmlResponse = await ai.models.generateContent({
                  model: 'gemini-2.5-flash',
                  contents: [{
                    role: 'user',
                    parts: [{
                      text: `Generate the final HTML content for an interactive ${documentType} about: ${refinedPrompt}

I have generated ${generatedImages.length} images for you to include. 
Requirements:
1. Use inline CSS for all styling (magazine style, modern).
2. Structure with Title Page, Table of Contents, and 3-6 Sections.
3. Insert the images using the following placeholders EXACTLY:
${planData.images?.map(img => `- For image "${img.caption}", use placeholder: {{${img.id}}}`).join('\n') || '- No images available'}

4. Wrap images in this HTML structure:
   <figure style="margin: 2rem 0; text-align: center;">
     <img src="{{img1}}" alt="Caption" style="max-width: 100%; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);" />
     <figcaption style="color: #666; font-size: 0.9em; margin-top: 0.5rem; font-style: italic;">[Caption]</figcaption>
   </figure>

5. Return ONLY the HTML code. No markdown code blocks, no JSON.`
                    }]
                  }],
                  config: {
                    temperature: 0.7,
                ...
                let finalHtml = htmlResponse.text || '';
                finalHtml = finalHtml.replace(/^```html\n?|^```\n?/i, '').replace(/\n?```$/i, '');

                // Replace placeholders
                for (const img of generatedImages) {
                  finalHtml = finalHtml.replace(new RegExp(`{{${img.id}}}`, 'g'), img.url);
                }

                // Cleanup unused placeholders
                if (planData.images) {
                  for (const img of planData.images) {
                    finalHtml = finalHtml.replace(new RegExp(`{{${img.id}}}`, 'g'), '');
                  }
                }

                if (finalHtml.length < 100) throw new Error('Failed to generate HTML content');

                addMessage('model', `💾 (Completed) Finalizing and saving...`);

                // Create a new BookAsset with isInteractive flag
                const pdfId = typeof crypto !== 'undefined' && (crypto as any).randomUUID
                  ? (crypto as any).randomUUID()
                  : `pdf-${Date.now()}`;

                // Note: latestSession/sessionId already defined above

                const bookAsset: BookAsset = {
                  id: pdfId,
                  title: prompt.slice(0, 100),
                  description: `Interactive ${documentType} - ${prompt.slice(0, 200)}`,
                  pages: [], // No image pages for interactive PDF
                  createdAt: Date.now(),
                  html: finalHtml,
                  isInteractive: true,
                };

                // Save to project's research session
                if (sessionId && latestSession?.researchReport) {
                  const existingBooks = latestSession.researchReport.books || [];
                  const updatedReport = {
                    ...latestSession.researchReport,
                ...
                const successMessage = `✅ **Interactive ${documentType.charAt(0).toUpperCase() + documentType.slice(1)} Generated!**

Your document "${bookAsset.title}" is ready with selectable text.

📖 **View & Download**: Go to **Assets → PDFs** tab, find your document, and enable "Interactive Mode" to view it.

*The document has rich HTML styling and can be printed directly from the viewer.*`;

                addMessage('model', successMessage);

              } catch (e: any) {
                console.error('generate_interactive_pdf error:', e);
                addMessage('model', `❌ Failed to generate interactive PDF: ${e?.message || 'Unknown error'}`);
              }
...
You can manage this product in **Assets → Products**.`;
          ...
          // Task Monitor: Clear pending task after tools finish executing
          completePendingTask();
        } else {
          // Fallback logic handled in the stream completion block above
        }

        // RE-ACT LOOP RECURSION
        if (toolOutputsForHistory.length > 0) {
          currentHistory.push({
            role: 'user', // Gemini 3 requires tool outputs to be 'user' role with functionResponse parts
            parts: toolOutputsForHistory
          });

          loopCount++;
          keepLooping = true;
          console.log(`[ProjectLiveAssistant] Re-Act Loop Triggered: ${loopCount}/${MAX_LOOPS}`);
        } else if (loopCount === 0 && initialNeedsBuiltInTools && !aggregatedFunctionCalls.length) {
          // If we just ran a Search iteration (Loop 0) and found no function calls (normal for Search),
          // FORCE continue to the next loop to allow Function Tools to run on the retrieved context.
          console.log('[ProjectLiveAssistant] Search turn complete (Loop 0). Continuing to allow Function Calls (Loop 1).');
          loopCount++;
          keepLooping = true;
        }
        ...
        <header className={`flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 border-b ${isDarkMode ? 'border-white/10 bg-black/5' : 'border-gray-200 bg-gray-50/50'}`}>
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className={`text-[10px] sm:text-xs truncate ${isDarkMode ? 'text-[#86868b]' : 'text-gray-500'}`}>
                {computeSourceCount(project)} sources
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {mode === 'voice' && connectionStatus === 'connected' ? (
              // Show End button when voice is connected
              <button
                          ...
                          <div className="flex flex-wrap gap-2">
                            {detectedPlatforms.map((platform) => {
                              const platformInfo: Record<SocialPlatform, { name: string; logo: string }> = {
                                facebook: { name: 'Facebook', logo: 'https://so7ow2fypz0vpnxi.public.blob.vercel-storage.com/2021_Facebook_icon.svg.webp' },
                                instagram: { name: 'Instagram', logo: 'https://so7ow2fypz0vpnxi.public.blob.vercel-storage.com/Instagram_logo_2016.svg.webp' },
                                x: { name: 'X', logo: 'https://so7ow2fypz0vpnxi.public.blob.vercel-storage.com/X-Logo-Round-Color.png' },
                                tiktok: { name: 'TikTok', logo: 'https://so7ow2fypz0vpnxi.public.blob.vercel-storage.com/tiktok-6338432_1280.webp' },
                                youtube: { name: 'YouTube', logo: 'https://so7ow2fypz0vpnxi.public.blob.vercel-storage.com/YouTube_full-color_icon_%282017%29.svg.png' },
                                linkedin: { name: 'LinkedIn', logo: 'https://so7ow2fypz0vpnxi.public.blob.vercel-storage.com/LinkedIn_logo_initials.png' },
                              };
                ...
                <div className="p-6 sm:p-8">
                  <div className="w-full flex flex-col items-center text-center space-y-5 sm:space-y-6">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 text-white font-medium py-3 px-5 sm:px-6 rounded-full transition-all active:scale-95 disabled:opacity-50 text-sm sm:text-base ${activeTheme === 'dark' || activeTheme === 'light'
